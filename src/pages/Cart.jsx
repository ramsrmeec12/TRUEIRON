...
export default function Cart() {
  ...
  const sendWhatsApp = (clientItems, clientName = '') => {
    const message = `Hi, I would like to enquire about the following products${clientName ? ` for ${clientName}` : ''}:
\n` +
      clientItems.map((item, i) => `${i + 1}. ${item.name} (SKU: ${item.sku || 'N/A'}), Qty: ${item.quantity}, Color: ${item.selectedColor}`).join('\n');
    ...
    <div className="p-4">
      ...
      {!userLoggedIn ? (
        <div className="space-y-4">
          ...
          {cartItems.length > 0 && (
            <div className="mt-4">
              <button onClick={() => sendWhatsApp(cartItems)} className="bg-black hover:bg-red-600 text-white px-6 py-2 rounded shadow-md">Submit Order via WhatsApp</button>
            </div>
          )}
        </div>
      ) : (
            ...
            <div key={client.id} className="mb-8 border rounded p-4 shadow">
              <details open>
                ...
                <div className="mt-4">
                  ...
                  <button onClick={() => sendWhatsApp(client.items, client.name)} className="ml-4 bg-black hover:bg-red-600 text-white px-6 py-2 rounded shadow-md">Submit Order via WhatsApp</button>
                </div>
              </details>
            </div>
          ))
